...
describes what users get. Pick a format with `-o`, or let olxvn choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
olxvn <command> -o table   # aligned columns for reading
olxvn <command> -o jsonl   # one JSON object per line, for piping
olxvn <command> -o json    # a single JSON array
olxvn <command> -o csv     # spreadsheet friendly
olxvn <command> -o tsv     # tab-separated
olxvn <command> -o url     # just the URL column
olxvn <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
olxvn <command> --fields id,title,url
```
...
```bash
olxvn <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
olxvn <command>            # a table, because this is a terminal
olxvn <command> | wc -l    # JSONL, because this is a pipe
```
